...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, (None, h, w, c * state_history))
        self.a = tf.placeholder(tf.int32, (None))
        self.r = tf.placeholder(tf.float32, (None))
        self.sp = tf.placeholder(tf.uint8, (None, h, w, c * state_history))
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        tq_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        ops = [tf.assign(tq_vars[i], q_vars[i]) for i in range(len(q_vars))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        default = self.r + self.config.gamma * tf.reduce_max(target_q, axis=1)
        q_samp = tf.where(self.done_mask, self.r, default)
        actions = tf.one_hot(self.a, num_actions)
        q_old = tf.reduce_sum(tf.multiply(actions, q), axis=1)
        self.loss = tf.reduce_mean(tf.square(q_samp - q_old))
        print(self.loss)
    ...
    def add_optimizer_op(self, scope):
        ...
        grads, vars = zip(*optimizer.compute_gradients(self.loss, vars))
        if self.config.grad_clip:
            clipped_grads = [tf.clip_by_norm(grad, self.config.clip_val) for grad in grads]
